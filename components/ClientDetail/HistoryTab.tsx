...
export default function HistoryTab({ history }: HistoryTabProps) {
  ...
  return (
    <div className="space-y-0">
      {history.map((entry, i) => {
        const isProposed = entry.actionType === 'rebalance_proposed'
        return (
          <div key={entry.id} className={`flex items-start gap-4 py-5 ${i < history.length - 1 ? 'border-b border-border/40' : ''}`}>
            <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-0.5 ${
              isProposed ? 'bg-[#FFF8F0] border border-[#D4800A]/30' : 'bg-surface-container'
            }`}>
              <span className={`material-symbols-outlined text-[16px] ${isProposed ? 'text-[#D4800A]' : 'text-sage'}`}>
                {isProposed ? 'pending' : 'history'}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  {isProposed && (
                    <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-[#FFF8F0] text-[#D4800A] border border-[#D4800A]/20">
                      Awaiting Approval
                    </span>
                  )}
                </div>
              ...
              <p className="text-[11px] text-sage mt-2">
                {isProposed ? 'Portfolio value at proposal: ' : 'Portfolio value at execution: '}
                <span className="font-medium text-text">{formatCurrency(entry.totalValue)}</span>
              </p>
            </div>
          </div>
        )
